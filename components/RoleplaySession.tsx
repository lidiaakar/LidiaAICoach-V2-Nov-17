...
import type { Message, LanguageCode, RoleplayScenario, AudioMetrics, SessionMaterial } from '../types';
...
const RoleplaySession: React.FC<RoleplaySessionProps> = ({ onEndSession, language, scenario, onBackToScenarios, sessionMaterial, customQuestions }) => {
  ...
  let systemInstructionTemplate = t(scenario.systemPromptKey);
  
  if (customQuestions) {
      const customQuestionInstruction = `
---------------------------------------------
USER-PROVIDED QUESTIONS
---------------------------------------------
CRITICAL INSTRUCTION: You MUST ask the user the following questions exactly as they are written, in the order they are provided. Ask them one by one. After the user answers a question, you may ask one or two brief, natural follow-up questions for clarification if needed, but your primary goal is to get through the user's list. Do not ask any other primary questions outside of this list.
...
---------------------------------------------
      `;
      systemInstructionTemplate = customQuestionInstruction + systemInstructionTemplate;
  }
  const systemInstruction = systemInstructionTemplate.replace('{languageName}', languageName);
  ...
  useEffect(() => {
    ...
    setMessages([{ role: 'assistant', content: "Let's begin the roleplay. I'm ready when you are. Click the microphone to start." }]);
  ...
  }, [language, scenario, t, systemInstruction, stopSession]);
  ...
  useEffect(() => {
    ...
    if (isGhostModeEnabled && currentUserUtterance.trim().split(' ').length > 5) { // Only trigger after a few words
        ghostModeTimerRef.current = window.setTimeout(async () => {
            try {
                const sessionContext = `The user is in a roleplay scenario: ${t(scenario.titleKey)}. They are playing the role of the user, and the assistant is playing its designated role.`;
  ...
  }, [currentUserUtterance, isGhostModeEnabled, language, sessionMaterial, scenario.titleKey, t]);
        ...
        if (audioTracks.length) {
            ...
            recorder.onstop = () => {
                ...
                reader.onloadend = () => {
                    ...
                    onEndSession(transcriptRef.current, base64data, audioMetrics, capturedFramesRef.current, null, sessionMaterial, t(scenario.titleKey), scenario);
                };
  ...
  }, [isLiveListening, startSession, stopAnalysis, onEndSession, sessionMaterial, t, scenario]);
  ...
  const handleStopRecording = useCallback(() => {
    ...
    if (mediaRecorderRef.current && isRecording) {
      ...
      onEndSession(transcriptRef.current, null, { pitchVariation: 0, volumeVariation: 0 }, [], null, sessionMaterial, t(scenario.titleKey), scenario);
    }
    setIsRecording(false);
  }, [isRecording, onEndSession, sessionMaterial, t, scenario]);
  ...
  return (
    <div className="max-w-3xl mx-auto bg-white rounded-xl shadow-2xl border border-gray-200/50 flex flex-col">
      <div className="p-4 border-b border-gray-200 flex justify-between items-center">
        <h2 className="text-xl font-bold text-center text-brand-dark flex-grow">{t('session.roleplay.title', { scenario: t(scenario.titleKey) })}</h2>
      ...
      <div className="p-4 text-center border-t border-gray-100 flex items-center justify-center flex-wrap gap-4">
        <button onClick={onBackToScenarios} className="text-gray-600 font-medium py-2 px-4 rounded-lg hover:bg-gray-100 transition-colors flex items-center gap-2">
